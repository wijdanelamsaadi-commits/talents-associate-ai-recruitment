...
$checks = array(
    ...
    'canonical AIOSEO conserve job_id' => strpos($plugin, "add_filter('aioseo_canonical_url', 'talents_candidate_bridge_filter_job_canonical_url')") !== false,
    'Open Graph AIOSEO conserve job_id' => strpos($plugin, "add_filter('aioseo_facebook_tags', 'talents_candidate_bridge_filter_job_open_graph_tags')") !== false,
    'Open Graph Yoast conserve job_id' => strpos($plugin, "add_filter('wpseo_opengraph_url', 'talents_candidate_bridge_filter_job_open_graph_url')") !== false,
    'URL partage détail reconstruite avec job_id' => strpos($plugin, 'talents_candidate_bridge_current_job_detail_share_url') !== false && strpos($plugin, 'talents_candidate_bridge_job_url(TALENTS_CANDIDATE_BRIDGE_DETAIL_SLUG, $job_id)') !== false,
    ...
    'soft skills utilisées comme compétences comportementales' => strpos($plugin, "'soft_skills', 'preferred_skills'") !== false,
    ...
    'compétences comportementales masquées si vides' => strpos($plugin, "if (!empty(\$job['preferred_skills']))") !== false,
