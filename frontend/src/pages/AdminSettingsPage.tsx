  ...
  return (
    <div className="space-y-6">
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        <p className="text-sm font-semibold uppercase tracking-wide text-[#EE6C2F]">Administration</p>
        <h2 className="mt-2 text-2xl font-semibold text-[#24303F]">Paramètres système</h2>
      ...
      <form className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm" onSubmit={handleSubmit}>
        <label className="block">
          ...
          <textarea
            className="mt-2 min-h-96 w-full rounded-lg border border-slate-300 p-4 font-mono text-sm outline-none focus:border-[#EE6C2F]"
            onChange={(event) => setSettingsText(event.target.value)}
            value={settingsText}
          />
        </label>
        <div className="mt-4 flex justify-end">
          <button className="rounded-lg bg-[#EE6C2F] px-4 py-2 text-sm font-semibold text-white disabled:opacity-60" disabled={isSaving} type="submit">
