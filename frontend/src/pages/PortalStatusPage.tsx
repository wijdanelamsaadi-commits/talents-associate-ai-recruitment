  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        <p className="text-sm font-semibold uppercase text-[#EE6C2F]">Suivi de candidature</p>
        <h1 className="mt-2 text-3xl font-semibold text-[#24303F]">Consulter le suivi de candidature</h1>
        ...
        <form className="mt-6 flex flex-col gap-3 sm:flex-row" onSubmit={handleSubmit}>
          <input
            className="min-w-0 flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-[#EE6C2F] focus:ring-2 focus:ring-[#EE6C2F]/20"
          ...
          />
          <button
            className="rounded-lg bg-[#EE6C2F] px-4 py-2 text-sm font-semibold text-white hover:bg-[#D9551B] disabled:cursor-not-allowed disabled:bg-slate-400"
            disabled={isLoading}
            type="submit"
          >
              ...
              {status.applications.map((application) => (
                <article className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm" key={application.application_id}>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <h2 className="text-lg font-semibold text-[#24303F]">{application.job_title}</h2>
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-lg bg-slate-50 p-3">
                      ...
                      <p className="mt-1 text-sm font-semibold capitalize text-[#24303F]">{formatLabel(application.current_stage)}</p>
                    </div>
                    <div className="rounded-lg bg-slate-50 p-3">
                      <p className="text-xs font-semibold uppercase text-slate-500">Date</p>
                      <p className="mt-1 text-sm font-semibold text-[#24303F]">
