  ...
  return (
    <main className="mx-auto grid max-w-6xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[1.15fr_0.85fr]">
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        <p className="text-sm font-semibold uppercase text-[#EE6C2F]">Profil candidat</p>
        <h1 className="mt-2 text-3xl font-semibold text-[#24303F]">Informations candidat</h1>
        ...
        <form className="mt-6 grid gap-4 sm:grid-cols-2" onSubmit={saveProfile}>
          ...
          <button className="rounded-lg bg-[#EE6C2F] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#D9551B] disabled:bg-slate-400 sm:col-span-2" disabled={isSaving} type="submit">
      ...
      <aside className="space-y-6">
        <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-xl font-semibold text-[#24303F]">Mon CV</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600">
            {profile?.latest_cv_filename ? `CV actuel : ${profile.latest_cv_filename}` : "Ajoutez votre CV pour postuler rapidement."}
          </p>
          <input
            accept=".pdf,.docx"
            className="mt-4 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm file:mr-3 file:rounded-md file:border-0 file:bg-[#EE6C2F]/10 file:px-3 file:py-1 file:text-sm file:font-semibold file:text-[#EE6C2F]"
            onChange={(event) => setFile(event.target.files?.[0] ?? null)}
            type="file"
          />
          <button className="mt-4 w-full rounded-lg bg-[#24303F] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#102b50] disabled:bg-slate-400" disabled={isSaving || !file} onClick={() => void uploadCv()} type="button">
            {profile?.latest_cv_filename ? "Remplacer mon CV" : "Déposer mon CV"}
          </button>
        </section>

        <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-xl font-semibold text-[#24303F]">Prochaine étape</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600">Consultez les opportunités et postulez avec votre CV déjà enregistré.</p>
          <Link className="mt-4 inline-flex w-full justify-center rounded-lg bg-[#EE6C2F] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#D9551B]" to="/portal/jobs">
            Voir les offres
          </Link>
        </section>
